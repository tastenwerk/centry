Centry.SessionsResetPasswordController = Ember.ObjectController.extend Centry.Validations,
  ...
  actions:
    ...
    resetPassword: ->
      ...
      Ember.$.post("#{Centry.get('apiHost')}/users/#{@get('content.id')}/reset_password", @getProperties('password', 'confirmation_key'))
        .then (response)=>
          Ember.$.ajaxSetup
